...
public class PlaceNameWorker implements ChildWorker<String, TextType> {
    ...
    public void execute() {
        if (itemIdList.isEmpty()) {
            log.debug("Nothing to write in table {}", PLACE_NAME.getName());
        } else {
            ctx.delete(PLACE_NAME)
               .where(PLACE_NAME.PLACE_ID.in(itemIdList))
               .execute();
