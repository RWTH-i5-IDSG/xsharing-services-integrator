...
import static jooq.db.ixsi.tables.FloatingAreaAttribute.FLOATING_AREA_ATTRIBUTE;
...
public class FloatingAreaAttributeWorker implements ChildWorker<String, String> {
    ...
    public void prepare() {
        insertBatch = ctx.batch(
                ctx.insertInto(FLOATING_AREA_ATTRIBUTE,
                        FLOATING_AREA_ATTRIBUTE.FLOATING_AREA_ID,
                        FLOATING_AREA_ATTRIBUTE.ATTRIBUTE_ID)
                        .values("", null)
        );
    }
    ...
    public void execute() {
        if (itemIdList.isEmpty()) {
            log.debug("Nothing to write in table {}", FLOATING_AREA_ATTRIBUTE.getName());
        } else {
            ctx.delete(FLOATING_AREA_ATTRIBUTE)
               .where(FLOATING_AREA_ATTRIBUTE.FLOATING_AREA_ID.in(itemIdList))
               .execute();
