...
import static jooq.db.ixsi.tables.PlaceDescription.PLACE_DESCRIPTION;
...
public class PlaceDescriptionWorker implements ChildWorker<String, TextType> {
    ...
    public void prepare() {
        insertBatch = ctx.batch(
                ctx.insertInto(PLACE_DESCRIPTION,
                        PLACE_DESCRIPTION.PLACE_ID,
                        PLACE_DESCRIPTION.LANGUAGE,
    ...
    public void execute() {
        if (itemIdList.isEmpty()) {
            log.debug("Nothing to write in table {}", PLACE_DESCRIPTION.getName());
        } else {
            ctx.delete(PLACE_DESCRIPTION)
               .where(PLACE_DESCRIPTION.PLACE_ID.in(itemIdList))
               .execute();
