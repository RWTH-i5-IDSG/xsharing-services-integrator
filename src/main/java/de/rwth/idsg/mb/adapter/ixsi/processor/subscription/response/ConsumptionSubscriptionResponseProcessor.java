 ...
 */
@Stateless
public class ConsumptionSubscriptionResponseProcessor
        implements SubscriptionResponseProcessor<ConsumptionSubscriptionRequestType,
                                                 ConsumptionSubscriptionResponseType> {
    ...
    @Override
    public Class<ConsumptionSubscriptionResponseType> getProcessingClass() {
        return ConsumptionSubscriptionResponseType.class;
    }

    @Override
    @ErrorLog
    public void process(InOutContext<ConsumptionSubscriptionRequestType,
                                     ConsumptionSubscriptionResponseType> context) {
        if (context.getIncoming().isSetError()) {
            return;
        }

        int partnerId = context.getPartnerContext().getConfig().getPartnerId();
        ConsumptionSubscriptionRequestType request = context.getOutgoing();
    ...
    private void optionalGetComplete(PartnerContext ctx) {
        if (ctx.getState().isSetup()) {
            subscriptionService.getCompleteConsumption(ctx, null);
        }
    }
}
